...
Puzzle: Advent of Code (year=2024 ; day=20 ; task=2)
"""
...
def main() -> None:
    ...
    target = 50 if len(grid) <= 15 else 100  # For test cases
    start, end = None, None
    ...
    #     then we can cheat between these two cells if their distance is less than 20
    print(
        sum(
            d1 + d + d2 + target <= total_dist
            for (r1, c1), d1 in dist_s.items()
            for (r2, c2), d2 in dist_e.items()
            if (d := abs(r1 - r2) + abs(c1 - c2)) <= 20
        ),
    )
