...
Puzzle: Advent of Code (year=2021 ; day=4 ; task=2)
"""
...
def main() -> None:
    ...
    for num in nums_to_call:  # call numbers
        i = 0
        while i < len(boards):  # mark them in every board
            if is_winning_number(boards[i], num):
                if len(boards) == 1:
                    print(get_board_sum(boards[i]) * int(num))
                    return
