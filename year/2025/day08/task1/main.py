...
Puzzle: Advent of Code (year=2025 ; day=8 ; task=1)
"""
...
def main() -> None:
    ...
    for _, p1, p2 in sorted(array)[: (1000 if len(points) > 20 else 10)]:
        dsu.join(p1, p2)

    sizes = sorted({dsu.get_set_size(i) for i in range(len(points))}, reverse=True)
    print(prod(sizes[:3]))
