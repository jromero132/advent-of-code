...
def solve(in_parsed) -> str:
    ans = 0
    for group in in_parsed:
        questions = set(chr(i) for i in range(ord('a'), ord('z') + 1))  # universe: all questions
        for answer in group.split("\n"):
            questions.intersection_update(answer)  # get common questions
        ans += len(questions)
