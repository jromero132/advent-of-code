...
def solve(in_parsed) -> str:
    valids = 0
    for policy, password in in_parsed:
        policy = policy.split()
        mi, ma = [int(x) for x in policy[0].split("-")]  # get positions
        valids += mi <= password.count(policy[1]) <= ma  # check if password contains at least 'mi' and at most 'ma'
    return str(valids)
