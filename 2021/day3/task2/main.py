...
def solve(in_parsed: list) -> str:
    ...
    def sim(op, vals):
        ...
        while len(vals) > 1:
            bit = get(op, i, vals)
            vals = [num for num in vals if num[i] == bit]
            i += 1
        return vals[0]

    co2 = int("".join(sim(min, in_parsed)), 2)  # get gamma value
    oxygen = int("".join(sim(max, in_parsed)), 2)  # get epsilon value

    return str(co2 * oxygen)  # get final value
