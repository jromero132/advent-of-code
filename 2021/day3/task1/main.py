...
def solve(in_parsed: list) -> str:
    def get(op, idx, vals):  # get (min, max) bit in index 'idx' based on the numbers in 'vals'
        cnt = sum(val[idx] == '0' for val in vals)
        return op((cnt, '0'), (len(vals) - cnt, '1'))[1]

    gamma = int("".join(get(min, i, in_parsed) for i in range(len(in_parsed[0]))), 2)  # get gamma value
    epsilon = int("".join(get(max, i, in_parsed) for i in range(len(in_parsed[0]))), 2)  # get epsilon value

    return str(gamma * epsilon)  # get final value
